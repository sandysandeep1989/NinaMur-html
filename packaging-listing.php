<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Packing List</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" href="./index.css">
    <link rel="stylesheet" href="./style.css">
    <link rel="stylesheet" href="./packaging-listing.css">
</head>

<body>

    <!-- Main Content -->
    <main id="main-content">

        <?php include 'header.php'; ?>

        <section class="pk-container">

            <!-- Title Row -->
            <div class="pk-title-row">
                <h1 class="pk-title">Packing List</h1>
                <span class="pk-results">Results: 37</span>
            </div>

            <!-- Action Buttons -->
            <div class="pk-action-row">
                <div class="pk-spacer"></div>
                <div class="pk-action-buttons">
                    <button class="pk-action-btn pk-action-btn--primary" title="Add New">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7.6 15.2C7.92727 15.2 8.18182 14.9455 8.18182 14.6182V8.18182H14.6909C15.0182 8.18182 15.2727 7.92727 15.2727 7.6C15.2727 7.27273 15.0182 7.01818 14.6909 7.01818H8.21818V0.581818C8.21818 0.290909 7.96364 0 7.63636 0C7.30909 0 7.05455 0.254546 7.05455 0.581818V7.01818H0.581818C0.254546 7.01818 0 7.27273 0 7.6C0 7.92727 0.254546 8.18182 0.581818 8.18182H7.01818V14.6182C7.05455 14.9455 7.30909 15.2 7.6 15.2Z" fill="white" />
                        </svg>
                    </button>
                    
                    <button class="pk-action-btn pk-action-btn--primary" title="Edit">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10.3843 0.354935C10.3967 0.354942 10.4102 0.355037 10.4243 0.356888L10.4233 0.357864C10.4674 0.362932 10.8152 0.408095 11.0571 0.649857L13.5132 3.10689C13.6961 3.29004 13.7675 3.53314 13.7935 3.65962L13.8071 3.74068V3.74165C13.8084 3.75414 13.8091 3.7672 13.8091 3.77974V14.4672C13.809 15.177 13.2317 15.7541 12.522 15.7543H3.49658C2.78663 15.7543 2.20958 15.1772 2.20947 14.4672V1.64204C2.20947 0.932028 2.78657 0.354935 3.49658 0.354935H10.3843ZM3.49658 1.02974C3.15899 1.02974 2.88428 1.30445 2.88428 1.64204V14.4672C2.88438 14.8047 3.15905 15.0795 3.49658 15.0795H12.522C12.8593 15.0793 13.1342 14.8046 13.1343 14.4672V4.11665H11.3345C10.6245 4.11665 10.0464 3.53956 10.0464 2.82954V1.02974H3.49658ZM10.7212 2.82954C10.7212 3.16714 10.9969 3.44185 11.3345 3.44185H12.894L10.7212 1.269V2.82954Z" fill="white" stroke="white" stroke-width="0.2" />
                        </svg>
                    </button>
                    <button class="pk-action-btn pk-action-btn--primary" title="Export">
                        <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0.668811 0.0818182L6.81427 6.22727V3.64546C6.81427 3.42727 6.99608 3.24545 7.21427 3.24545C7.43245 3.24545 7.61427 3.42727 7.61427 3.64546V7.20909C7.61427 7.42727 7.43245 7.60909 7.21427 7.60909H3.65063C3.43245 7.60909 3.25063 7.42727 3.25063 7.20909C3.25063 6.99091 3.43245 6.80909 3.65063 6.80909H6.23245L0.0869966 0.663637C0.0142693 0.590909 -0.0220988 0.481818 0.0142648 0.372727C0.0142648 0.263636 0.0869954 0.154545 0.159723 0.0818182C0.305177 -0.0272727 0.523357 -0.0272727 0.668811 0.0818182Z" fill="white" />
                            <path d="M15.0323 14.5911C15.0323 14.8093 14.8504 14.9911 14.6323 14.9911H1.50498C1.2868 14.9911 1.10498 14.8093 1.10498 14.5911V6.11839C1.10498 5.90021 1.2868 5.71839 1.50498 5.71839C1.72316 5.71839 1.90498 5.90021 1.90498 6.11839V14.1911H14.2323V1.86384H6.15953C5.94135 1.86384 5.75953 1.68203 5.75953 1.46384C5.75953 1.24566 5.94135 1.06384 6.15953 1.06384H14.5959C14.8141 1.06384 14.9959 1.24566 14.9959 1.46384V14.5911H15.0323Z" fill="white" />
                        </svg>
                    </button>
                    <button class="pk-action-btn pk-action-btn--primary" title="Delete">
                        <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M8.07262 7.52736L14.2181 1.38191V3.96373C14.2181 4.18191 14.3999 4.36373 14.6181 4.36373C14.8363 4.36373 15.0181 4.18191 15.0181 3.96373V0.400092C15.0181 0.18191 14.8363 9.15527e-05 14.6181 9.15527e-05H11.0544C10.8363 9.15527e-05 10.6544 0.18191 10.6544 0.400092C10.6544 0.618274 10.8363 0.800092 11.0544 0.800092H13.6362L7.4908 6.94555C7.41807 7.01827 7.38171 7.12736 7.41808 7.23646C7.41808 7.34555 7.4908 7.45464 7.56353 7.52736C7.70898 7.63646 7.92716 7.63646 8.07262 7.52736Z" fill="white" />
                            <path d="M0 14.5821C0 14.8002 0.18182 14.9821 0.400002 14.9821H13.5273C13.7455 14.9821 13.9273 14.8002 13.9273 14.5821V6.10933C13.9273 5.89114 13.7455 5.70932 13.5273 5.70932C13.3091 5.70932 13.1273 5.89114 13.1273 6.10933V14.1821H0.800004V1.85478H8.87274C9.09092 1.85478 9.27273 1.67296 9.27273 1.45478C9.27273 1.2366 9.09092 1.05478 8.87274 1.05478H0.436368C0.218186 1.05478 0.0363659 1.2366 0.0363659 1.45478V14.5821H0Z" fill="white" />
                        </svg>
                    </button>
                    <button class="pk-action-btn pk-action-btn--primary" title="Download">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12.1091 2.36364C12 1.92727 11.7818 1.52727 11.4182 1.23636C11.0545 0.945454 10.6182 0.8 10.1818 0.8H9.41818C8.98182 0.8 8.54545 0.945454 8.18182 1.23636C7.81818 1.52727 7.6 1.92727 7.49091 2.36364H12.1091ZM0.400002 2.36364H6.72727C6.8 1.78182 7.05455 1.23636 7.49091 0.8C7.96364 0.290909 8.69091 0 9.41818 0H10.2182C10.9455 0 11.6364 0.290909 12.1455 0.8C12.5818 1.23636 12.8364 1.78182 12.9091 2.36364H18.8364C19.0545 2.36364 19.2364 2.54545 19.2364 2.76364C19.2364 2.98182 19.0545 3.16364 18.8364 3.16364H12.5455H7.05454H0.400002C0.18182 3.16364 0 2.98182 0 2.76364C0 2.54545 0.18182 2.36364 0.400002 2.36364Z" fill="white"/>
                        <path d="M2.28987 3.92741C2.50805 3.89105 2.68988 4.0365 2.72624 4.25469L4.32624 16.8001C4.39896 17.4547 4.72624 18.0729 5.23533 18.5092C5.74442 18.9456 6.3626 19.2001 7.05351 19.2001H12.5444C13.199 19.2001 13.8535 18.9456 14.3626 18.5092C14.8717 18.0729 15.199 17.4547 15.2717 16.8001L16.8717 4.25469C16.9081 4.0365 17.0899 3.89105 17.3081 3.92741C17.5262 3.96378 17.6717 4.1456 17.6353 4.36378L16.0353 16.9092C15.9262 17.7456 15.5262 18.5456 14.8717 19.0911C14.2171 19.6729 13.4171 19.9638 12.5444 19.9638H7.05351C6.18078 19.9638 5.38078 19.6365 4.72624 19.0911C4.07169 18.5092 3.67169 17.7456 3.5626 16.9092L1.9626 4.36378C1.92624 4.1456 2.07169 3.96378 2.28987 3.92741Z" fill="white"/>
                        <path d="M9.41833 15.6V7.45457L7.70923 9.16366C7.56378 9.30911 7.30923 9.30911 7.16378 9.16366C7.01832 9.0182 7.01832 8.76366 7.16378 8.6182L9.52741 6.25457C9.67287 6.10911 9.92741 6.10911 10.0729 6.25457L12.4365 8.6182C12.582 8.76366 12.582 9.0182 12.4365 9.16366C12.2911 9.30911 12.0365 9.30911 11.8911 9.16366L10.182 7.45457V15.6C10.182 15.7091 10.1092 15.7818 10.0365 15.8546C9.96379 15.9273 9.85469 15.9637 9.7456 15.9637C9.60014 15.9637 9.45469 15.8182 9.41833 15.6Z" fill="white"/>
                        </svg>

                    </button>
                </div>
            </div>

            <!-- Packing List Table -->
            <div class="pk-table-wrapper">
                <table class="pk-table">

                    <colgroup>
                        <col class="pk-col-id">
                        <col class="pk-col-timestamp">
                        <col class="pk-col-related">
                        <col class="pk-col-select">
                        <col class="pk-col-actions">
                    </colgroup>

                    <thead>
                        <tr>
                            <th class="pk-th-sortable">
                                <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                ID
                            </th>
                            <th class="pk-th-sortable">
                                <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                TIMESTAMP
                            </th>
                            <th class="pk-th-sortable">
                                <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                RELATED ITEMS/ORDERS AND CUSTOMERS
                            </th>
                            <th class="pk-th-center pk-th-clickable" id="selectHeader">SELECT</th>
                            <th class="pk-th-sortable pk-th-center" id="actionsHeader">
                                <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                ACTIONS
                                <!-- Delete All Popover -->
                                <div class="pk-popover" id="actionsPopover">
                                    <button class="pk-popover-item" id="deleteAllBtn">
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13.3332 4L5.99984 11.3333L2.6665 8" stroke="#020617" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                        Delete All
                                    </button>
                                </div>
                            </th>
                        </tr>
                    </thead>

                    <tbody>
                        <?php
                        $packingData = [
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Fernanda Pink Blush A67 - 50°18 (oi10688) - Eye Gotcha Optometric',
                                    'Elia Emerald Green A03 - 49°18 (oi10709) - Eye Gotcha Optometric'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Rafaela Turquoise Green A32 W/ Transparent Nosepad - 50°21 (oi10668) - Eye Spy Optical',
                                    'Elena Black Soft A74 W/ Transparent Nosepad - 48°23,5 (+1mm) (oi10741) - Eye Spy Optical'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Maiko Oxid Copper A46 W/ Titanium Nosepad - 48°21 - 135mm (oi10691) - A Proper View',
                                    'Monica Deep Purple A49 - 53°17 (oi10708) - A Proper View'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Antonio Blue Steel A25 - 50°21 (oi10739) - Taylor West And Co Hove',
                                    'Alvaro 9mm Red Wine B08 - 46°24 (oi10739) - Taylor West And Co Hove',
                                    'Astor Fluo Yellow A58 - 52°23 (oi10745) - Taylor West And Co Hove'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Fernanda Pink Blush A67 - 50°18 (oi10688) - Eye Gotcha Optometric',
                                    'Elia Emerald Green A03 - 49°18 (oi10709) - Eye Gotcha Optometric'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Miriam Mercury B17 - 53°21 (oi10389) - Carlos Venegas',
                                    'Stefania Saturn B36 - 53°18 (oi10389) - Carlos Venegas',
                                    'Simone Moon B19 - 54°19 (oi10389) - Carlos Venegas',
                                    'Francesca Mars B15 - 57°20 (oi10389) - Carlos Venegas'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Fernanda Pink Blush A67 - 50°18 (oi10688) - Eye Gotcha Optometric',
                                    'Elia Emerald Green A03 - 49°18 (oi10709) - Eye Gotcha Optometric'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Fernanda Pink Blush A67 - 50°18 (oi10688) - Eye Gotcha Optometric',
                                    'Elia Emerald Green A03 - 49°18 (oi10709) - Eye Gotcha Optometric'
                                ]
                            ],
                            [
                                'id' => 'PL0036',
                                'timestamp' => '02-03-2026 11:15:44 AM',
                                'items' => [
                                    'Fernanda Pink Blush A67 - 50°18 (oi10688) - Eye Gotcha Optometric',
                                    'Elia Emerald Green A03 - 49°18 (oi10709) - Eye Gotcha Optometric'
                                ]
                            ],
                        ];

                        foreach ($packingData as $row):
                        ?>
                        <tr>
                            <td><a class="pk-id-link" href="#"><?php echo htmlspecialchars($row['id']); ?></a></td>
                            <td><?php echo htmlspecialchars($row['timestamp']); ?></td>
                            <td class="pk-td-related">
                                <div class="pk-related-text">
                                    <?php foreach ($row['items'] as $item): ?>
                                        <span><?php echo htmlspecialchars($item); ?></span>
                                    <?php endforeach; ?>
                                </div>
                            </td>
                            <td class="pk-td-center">
                                <div class="pk-select-cell">
                                    <img src="images/icon-menu.svg" width="14" height="13" alt="Select">
                                </div>
                            </td>
                            <td>
                                <div class="pk-row-actions">
                                    <button class="pk-row-action-btn pk-row-action-btn--copy" title="Copy">
                                        <img src="images/icon-copy.svg" width="12" height="12" alt="Copy">
                                    </button>
                                    <button class="pk-row-action-btn pk-row-action-btn--folder" title="Folder">
                                        <img src="images/icon-folder.svg" width="12" height="12" alt="Folder">
                                    </button>
                                    <button class="pk-row-action-btn pk-row-action-btn--delete" title="Delete">
                                        <img src="images/icon-trash.svg" width="12" height="12" alt="Delete">
                                    </button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>

                </table>
            </div>

        </section>

        <!-- Selected Items Modal -->
        <div class="pk-overlay" id="pkSelectedOverlay">
            <div class="pk-modal pk-modal--wide">

                <!-- Header -->
                <div class="pk-modal-header">
                    <h2 class="pk-modal-title">Selected Items</h2>
                    <button class="pk-modal-close" id="pkSelectedClose">
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                            <path d="M1 1L11 11M11 1L1 11" stroke="#fff" stroke-width="2" stroke-linecap="round"/>
                        </svg>
                    </button>
                </div>

                <!-- Table -->
                <div class="pk-modal-table-wrap">
                    <table class="pk-modal-table">
                        <colgroup>
                            <col style="width:50px">
                            <col style="width:150px">
                            <col style="width:39px">
                            <col style="width:200px">
                            <col style="width:80px">
                            <col style="width:30px">
                            <col style="width:30px">
                            <col style="width:30px">
                            <col style="width:30px">
                            <col style="width:80px">
                            <col style="width:48px">
                            <col style="width:80px">
                            <col style="width:62px">
                            <col style="width:90px">
                            <col style="width:62px">
                            <col style="width:38px">
                            <col style="width:38px">
                            <col style="width:120px">
                            <col style="width:90px">
                            <col style="width:64px">
                            <col style="width:40px">
                            <col style="width:40px">
                            <col style="width:40px">
                        </colgroup>
                        <thead>
                            <tr>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    ID
                                </th>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    PRODUCT NAME
                                </th>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    QTY
                                </th>
                                <th class="pk-modal-center">PREVIEW</th>
                                <th class="pk-modal-underline">PACKAGING</th>
                                <th class="pk-modal-rotated"><span>EXTRA INFO</span></th>
                                <th class="pk-modal-rotated"><span>LASER ENGRAV.</span></th>
                                <th class="pk-modal-rotated">
                                    <span>
                                        <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                        LABEL NAME
                                    </span>
                                </th>
                                <th class="pk-modal-rotated"><span>CALIBRE</span></th>
                                <th class="pk-modal-center">
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    PRICE
                                </th>
                                <th class="pk-modal-rotated">
                                    <span>
                                        <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                        ORDER ID
                                    </span>
                                </th>
                                <th class="pk-modal-center">
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    SHIPPING
                                </th>
                                <th class="pk-modal-rotated pk-modal-rotated--tall">
                                    <span>INCLUDE SHIPPING IN COMBINED INVOICE</span>
                                </th>
                                <th class="pk-modal-center">
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    ORDER EXTRAS
                                </th>
                                <th class="pk-modal-rotated pk-modal-rotated--tall">
                                    <span>INCLUDE EXTRAS IN COMBINED INVOICE</span>
                                </th>
                                <th>P1</th>
                                <th>P2</th>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    CUSTOMER NAME
                                </th>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    INVOICE ID
                                </th>
                                <th>
                                    <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                    SHIPPED
                                </th>
                                <th class="pk-modal-rotated"><span>LASER ENGRAV.</span></th>
                                <th class="pk-modal-rotated">
                                    <span>
                                        <img class="pk-sort-icon" src="images/icon-sort.svg" width="16" height="16" alt="Sort">
                                        LABEL NAME
                                    </span>
                                </th>
                                <th class="pk-modal-rotated"><span class="pk-modal-underline">ACTIONS</span></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $selectedItems = [
                                ['id' => 'PI0201', 'name' => 'DARIO DARK GREEN w/ TITANIUM Nosepad', 'qty' => 2, 'pack' => 'Full Pack', 'price' => '1,50,000€', 'orderId' => '0110688', 'shipping' => '1,000€', 'extras' => '1,000€', 'p1' => '#B7DBA4', 'p2' => '#B7DBA4', 'customer' => 'CENTRO ÓPTICO', 'invoice' => 'F_2029_28_12'],
                                ['id' => 'PI0201', 'name' => 'DARIO DARK GREEN w/ TITANIUM Nosepad', 'qty' => 2, 'pack' => 'Full Pack', 'price' => '1,50,000€', 'orderId' => '0110688', 'shipping' => '1,000€', 'extras' => '1,000€', 'p1' => '#B7DBA4', 'p2' => '#FF9F9C', 'customer' => 'CENTRO ÓPTICO', 'invoice' => 'F_2029_28_12'],
                                ['id' => 'PI0201', 'name' => 'DARIO DARK GREEN w/ TITANIUM Nosepad', 'qty' => 2, 'pack' => 'Full Pack', 'price' => '1,50,000€', 'orderId' => '0110688', 'shipping' => '1,000€', 'extras' => '1,000€', 'p1' => '#B7DBA4', 'p2' => '#B7DBA4', 'customer' => 'CENTRO ÓPTICO', 'invoice' => 'F_2029_28_12'],
                                ['id' => 'PI0201', 'name' => 'DARIO DARK GREEN w/ TITANIUM Nosepad', 'qty' => 2, 'pack' => 'Full Pack', 'price' => '1,50,000€', 'orderId' => '0110688', 'shipping' => '1,000€', 'extras' => '1,000€', 'p1' => '#FF9F9C', 'p2' => '#FF9F9C', 'customer' => 'CENTRO ÓPTICO', 'invoice' => 'F_2029_28_12'],
                            ];

                            foreach ($selectedItems as $item):
                            ?>
                            <tr>
                                <td><?php echo htmlspecialchars($item['id']); ?></td>
                                <td class="pk-modal-product"><a class="pk-modal-product-link" href="#"><?php echo htmlspecialchars($item['name']); ?></a></td>
                                <td class="pk-modal-center"><?php echo $item['qty']; ?></td>
                                <td class="pk-modal-center"><img class="pk-modal-preview" src="images/preview-glasses.png" alt="Preview"></td>
                                <td>
                                    <select class="pk-modal-pack-select">
                                        <option <?php if ($item['pack'] === 'Full Pack') echo 'selected'; ?>>Full Pack</option>
                                        <option <?php if ($item['pack'] === 'Soft Pack') echo 'selected'; ?>>Soft Pack</option>
                                    </select>
                                </td>
                                <td class="pk-modal-center"><img src="images/icon-info.svg" width="16" height="16" alt="Info"></td>
                                <td class="pk-modal-center"><img src="images/icon-info.svg" width="16" height="16" alt="Info"></td>
                                <td class="pk-modal-center"><img src="images/icon-info.svg" width="16" height="16" alt="Info"></td>
                                <td class="pk-modal-center">-</td>
                                <td class="pk-modal-center"><?php echo htmlspecialchars($item['price']); ?></td>
                                <td class="pk-modal-center"><a class="pk-modal-order-link" href="#"><?php echo htmlspecialchars($item['orderId']); ?></a></td>
                                <td class="pk-modal-center"><?php echo htmlspecialchars($item['shipping']); ?></td>
                                <td class="pk-modal-center"><input type="checkbox" class="pk-modal-checkbox"></td>
                                <td class="pk-modal-center"><?php echo htmlspecialchars($item['extras']); ?></td>
                                <td class="pk-modal-center"><input type="checkbox" class="pk-modal-checkbox"></td>
                                <td class="pk-modal-p-cell" style="background:<?php echo $item['p1']; ?>">&nbsp;</td>
                                <td class="pk-modal-p-cell" style="background:<?php echo $item['p2']; ?>">&nbsp;</td>
                                <td><?php echo htmlspecialchars($item['customer']); ?></td>
                                <td><?php echo htmlspecialchars($item['invoice']); ?></td>
                                <td class="pk-modal-center"><img src="images/icon-folder.svg" width="18" height="18" alt="Shipped"></td>
                                <td class="pk-modal-center"><img src="images/icon-info.svg" width="14" height="14" alt="Info"></td>
                                <td class="pk-modal-center"><img src="images/icon-info.svg" width="14" height="14" alt="Info"></td>
                                <td class="pk-modal-center">
                                    <button class="pk-modal-trash-btn">
                                        <img src="images/icon-trash.svg" width="10" height="10" alt="Delete">
                                    </button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>

                <!-- Footer -->
                <div class="pk-modal-footer">
                    <div class="pk-modal-totals">
                        <div class="pk-modal-total-row">
                            <span>Total Selected for Prof. Invoice:</span>
                            <span class="pk-modal-total-value">1,50,000€</span>
                        </div>
                        <div class="pk-modal-total-row">
                            <span>Total Selected for Comb. Invoice:</span>
                            <span class="pk-modal-total-value">1,50,000€</span>
                        </div>
                        <div class="pk-modal-total-row">
                            <span>Shipping Cost for Comb. Invoice:</span>
                            <span class="pk-modal-total-value">1,50,000€</span>
                        </div>
                        <div class="pk-modal-total-row">
                            <span>Extras Cost for Comb. Invoice:</span>
                            <span class="pk-modal-total-value">1,50,000€</span>
                        </div>
                    </div>
                    <div class="pk-modal-buttons">
                        <button class="pk-modal-action-btn">GENERATE PROFORMA INVOICE DOC</button>
                        <button class="pk-modal-action-btn">GENERATE COMBINED INVOICE</button>
                    </div>
                </div>

            </div>
        </div>

        <?php include 'footer.php'; ?>

    </main>

    <script src="./script.js"></script>
    <script src="./indexx.js"></script>
    <script src="./packaging-listing.js"></script>

</body>

</html>
